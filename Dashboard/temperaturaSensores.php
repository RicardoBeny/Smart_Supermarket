...
<?php
    ...
    if( filesize("API/Files/TemperaturaFrigorifico/valor.txt") == FALSE){

        $valor_frig = "N/A";

    } else {
        $valor_frig = file_get_contents("API/Files/TemperaturaFrigorifico/valor.txt"); 
        $valor_frig = openssl_decrypt($valor_frig, $cipher, $key, $options=0, $iv); /* decrypt valor */
    }
    
    if( filesize("API/Files/TemperaturaFrigorifico/hora.txt") == FALSE ){

        $hora_frig = "Indisponivel";

    }else {
        $hora_frig = file_get_contents("API/Files/TemperaturaFrigorifico/hora.txt"); 
        $hora_frig = openssl_decrypt($hora_frig, $cipher, $key, $options=0, $iv);
    }
    
    if( filesize("API/Files/TemperaturaCongelador/valor.txt") == FALSE ){

        $valor_cong = "N/A";

    }else {
        $valor_cong = file_get_contents("API/Files/TemperaturaCongelador/valor.txt"); 
        $valor_cong = openssl_decrypt($valor_cong, $cipher, $key, $options=0, $iv);
    }

    if( filesize("API/Files/TemperaturaCongelador/hora.txt") == FALSE ){

        $hora_cong = "Indisponivel";

    }else {
        $hora_cong = file_get_contents("API/Files/TemperaturaCongelador/hora.txt"); 
        $hora_cong = openssl_decrypt($hora_cong, $cipher, $key, $options=0, $iv);
    }
...
<head>
    ...
    <link rel="stylesheet" href="tempSens.css">
    ...
    <div class="topbar">
        ...
        <div class="cardsMainPage">
            <div class="card shadow-none rounded-0" >
                <div class="card-top-frig"> <!-- CARD FRIGORIFICO -->
                    <h1 class="heading">Frigorifico</h1>
                    <p class="temp">
                        <span class="temp-value"><?php echo ($valor_frig)?>ºC</span>
                    </p>
                </div>
                <div class="card-body">
                    <p class="card-text text-center">Atualização : <?php echo ($hora_frig)?></p>
                    <a href="API/historico.php?nome=TemperaturaFrigorifico" style="text-decoration: none">
                        <div class="card">
            ...
            <div class="card shadow-none rounded-0" style="width: 450px; height: 750px">
                <div class="card-top-cong"> <!-- CARD CONGELADOR -->
                <h1 class="heading">Congelador</h1>
                <p class="temp">
                    <span class="temp-value"><?php echo ($valor_cong)?>ºC</span>
                </p>
                </div>
                <div class="card-body">
                    <p class="card-text text-center">Atualização : <?php echo ($hora_cong)?></p>
                    <a href="API/historico.php?nome=TemperaturaCongelador" style="text-decoration: none">
                        <div class="card">
                                <span class="historicoText text-center">HISTORICO</span>
                        </div>
                    </a>
